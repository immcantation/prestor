...
# Check files
chunk_eval <- c("front"=TRUE)
chunk_eval["console"] <- file.exists(file.path(params$data, params$console))
chunk_eval["quality"] <- all(file.exists(file.path(params$data, params$quality_r1)),
                            file.exists(file.path(params$data, params$quality_r2)))
chunk_eval["primers"] <- all(file.exists(file.path(params$data, params$primers_r1)),
                            file.exists(file.path(params$data, params$primers_r2)))
chunk_eval["consensus"] <- all(file.exists(file.path(params$data, params$consensus_r1)),
                              file.exists(file.path(params$data, params$consensus_r2)))
chunk_eval["assemble"] <- file.exists(file.path(params$data, params$assemble))
chunk_eval["cregion"] <- file.exists(file.path(params$data, params$cregion))
chunk_eval["headers"] <- all(file.exists(file.path(params$data, params$headers_total)),
...
eval_func <- function() {
     chunk_eval[opts_current$get("label")]
}
```

```{r front, child="Front.Rmd", eval=eval_func()}
```

```{r console, child="Paired-Console.Rmd", eval=eval_func()}
```

```{r quality, child="Paired-Quality.Rmd", eval=eval_func()}
```

```{r primers, child="Paired-Primers.Rmd", eval=eval_func()}
```

```{r consensus, child="Paired-Consensus.Rmd", eval=eval_func()}
```
 
```{r assemble, child="Assembly.Rmd", eval=eval_func()}
```

```{r cregion, child="CRegion.Rmd", eval=eval_func()}
```
 
```{r headers, child="Headers.Rmd", eval=eval_func()}
```